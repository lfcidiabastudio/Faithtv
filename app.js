...
function commonHero(s,label,date,sub){$("heroLabel").textContent=label;$("heroTitle").textContent="Week of Spiritual Emphasis (DAY 3)";$("heroDate").textContent=date;$("heroService").textContent=sub;$("playerTitle").textContent="Week of Spiritual Emphasis (DAY 3)";$("ngClock").textContent=new Intl.DateTimeFormat("en-NG",{timeZone:TZ,hour:"2-digit",minute:"2-digit"}).format(new Date());}
function setLive(s){
 setBodyState("live");setHeaderState(true);commonHero(s,"WE'RE LIVE",`${timeLabel(s.start)} – ${timeLabel(s.end)}`,`Week of Spiritual Emphasis (DAY 3) · On air now`);
