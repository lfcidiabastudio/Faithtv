...
function commonHero(s,label,date,sub){$("heroLabel").textContent=label;$("heroTitle").textContent=s.title;$("heroDate").textContent=date;$("heroService").textContent=sub;$("playerTitle").textContent=s.day===0?"Covenant Day of Long Life":s.title;$("ngClock").textContent=new Intl.DateTimeFormat("en-NG",{timeZone:TZ,hour:"2-digit",minute:"2-digit"}).format(new Date());}
function setLive(s){
