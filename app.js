...
function displayName(s){
 if(!s)return s.title;
 if(s.day===0){const n=window.FAITHTVContent&&window.FAITHTVContent.getSundayName();if(n)return n;}
 if(s.day===3){const n=window.FAITHTVContent&&window.FAITHTVContent.getWednesdayName();if(n)return n;}
 return s.title;
}
function commonHero(s,label,date,sub){
 const name=displayName(s);
 $("heroLabel").textContent=label;$("heroTitle").textContent=name;$("heroDate").textContent=date;$("heroService").textContent=sub;
 $("playerTitle").textContent=s.day===0?"Miracle Banquet Service":name;
 $("ngClock").textContent=new Intl.DateTimeFormat("en-NG",{timeZone:TZ,hour:"2-digit",minute:"2-digit"}).format(new Date());
}
function setLive(s){
